...
// ==================================================
// Check if Satellite class already exists to prevent redeclaration
    ...
    class Satellite {
        ...
        groundTrace(time = new Date()) {
            ...
            try {
                ...
                const positionGd = satellite.eciToGeodetic(positionEci, gmst);
                // *** REMOVE Log Geodetic position object ***
                ...
                // *** REMOVE Log calculated Lat/Lon/Height before final validation ***
        ...
        updateMapDisplay(date) {
            if (!this.satrec || !this.layerGroup) {
                console.warn(`Cannot update map for satellite ${this.satID}: invalid satrec or missing layer group`);
                return;
            }

            try {
                // Calculate current position
                const position = this.calculatePositionAt(date);

                // Update marker position if we have a valid position
                if (position && this.marker) {
                    this.marker.setLatLng([position.lat, position.lng]);
                    this.marker.bindPopup(`<b>${this.satName}</b><br>Lat: ${position.lat.toFixed(2)}°<br>Lon: ${position.lng.toFixed(2)}°<br>Alt: ${position.alt.toFixed(1)} km`);
                }

                // Calculate and update ground trace
                this.updateGroundTrace(date);

                // Update information in the table
                this.updatePositionDisplay(date);
            } catch (error) {
                console.error(`Error updating map display for satellite ${this.satID}:`, error);
            }
        }

        updateGroundTrace(date) {
            if (!this.satrec || !this.groundTrack) return;

            // Calculate ground trace points (about 1 orbit ahead)
            const trace = groundTrace(this.satrec, date, 60); // 60 points

            // Handle the case where the ground trace crosses the antimeridian
            const processedCoords = [];
            let lastLon = null;

            for (let i = 0; i < trace.length; i++) {
                const [lon, lat] = trace[i];

                // Check for antimeridian crossing
                if (lastLon !== null && Math.abs(lon - lastLon) > 180) {
                    // Add the point nearest to the edge
                    if (lastLon < 0) {
                        processedCoords.push([-180, trace[i - 1][1]]);
                        processedCoords.push([180, lat]);
                    } else {
                        processedCoords.push([180, trace[i - 1][1]]);
                        processedCoords.push([-180, lat]);
                    }
            ...
            // Update polyline with processed coordinates
            this.groundTrack.setLatLngs(processedCoords.map(coord => [coord[1], coord[0]]));
        }
    }

    // Make Satellite available globally
    window.Satellite = Satellite;
} else {
    console.log("Satellite class already exists, skipping redeclaration");
}
// ==================================================
